...
void chip_update_bend()
{
	...
	if(tick_208 && (!m_rom_pause || !(m_filt_fa || m_filt_va))) {
	  ...
	  m_cur_f1=m_cur_f1_orig+(128-(exy[_intmode][2]*256.0f)); // TESTING!
	  m_cur_f2=m_cur_f2_orig+(64-(exy[_intmode][3]*128.0f)); // TESTING!
	  m_cur_f3=m_cur_f3_orig+(64-(exy[_intmode][4]*128.0f)); // TESTING!
	  m_cur_f2q=m_cur_f2q_orig+(64-(exy[_intmode][5]*128.0f)); // TESTING!
	  m_cur_fc=m_cur_fc_orig+(64-(exy[_intmode][6]*128.0f)); // TESTING!
	}
	...
	if(tick_625) {
	  if(m_ticks >= m_rom_vd){
		  ...
		  m_cur_fa=m_cur_fa_orig+(64-(exy[_intmode][7]*128.0f)); // TESTING!
	  }
		if(m_ticks >= m_rom_cld){
			//          interpolate(m_cur_fc, m_rom_fc);
				  
		  m_cur_va_orig=interpolate(m_cur_va_orig, m_rom_va);
		  m_cur_va=m_cur_va_orig+(64-(exy[_intmode][1]*128.0f)); // TESTING!
	...
	//	if(m_pitch == (0x7f ^ (m_inflection << 4) ^ (m_filt_f1+((int)((1.0f-exy[_intmode][0])*64.0f)-8)) + 1)) m_pitch = 0;
	u8 val=exy[_intmode][0]*131.0f;
	MAXED(val,127);
	val=127-val;
...
{
  m_cur_f1=(1.0f-exy[_intmode][1])*255.0f;
  m_cur_f2=(1.0f-exy[_intmode][2])*255.0f;
  m_cur_f3=(1.0f-exy[_intmode][3])*255.0f;
  m_cur_f2q=(1.0f-exy[_intmode][4])*255.0f;
  m_cur_fc=(1.0f-exy[_intmode][5])*255.0f;
  m_cur_fa=(1.0f-exy[_intmode][6])*255.0f;
  m_cur_va=(1.0f-exy[_intmode][0])*255.0f;
...
u32 analog_calc()
{
	...
	//	vn = apply_filter(m_vn_1, m_vn_2, m_f3_a, m_f3_b,4,4); // was removed - reverted
	...
	//	vn = apply_filter(m_vn_3, m_vn_4, m_f4_a, m_f4_b,4,4); // was removed - reverted
